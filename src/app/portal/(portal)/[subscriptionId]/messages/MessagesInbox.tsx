...
import type { SmsQuota } from "@/lib/sms-quota";
...
export default function MessagesInbox({ subscriptionId, initialThreads, initialReviewId, smsQuota, lang }: Props) {
  ...
  const [selectedReviewId, setSelectedReviewId] = useState<number | null>(
    initialReviewId && initialThreads.some((th) => th.review_id === initialReviewId)
      ? initialReviewId
      : initialThreads[0]?.review_id ?? null
  );
  ...
  const smsQuotaBar = (
    <p className="text-xs text-gray-400 dark:text-gray-500 mb-2 text-right">
      {t("portal_sms_quota_label", lang)}: {smsQuota.used}/{smsQuota.limit}
    </p>
  );
